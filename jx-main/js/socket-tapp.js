...
const updatePlayercount = (ip, port) => {
    $.get(`https://api.binance.com/api/v3/ticker/price?symbol=BTCUSDT#?id=${ip}&${port}`, (result) => {
...
const updatePlayercounts = (ip, port) => {
    $.get(`https://api.binance.com/api/v3/ticker/price?symbol=ETHUSDT#?id=${ip}&${port}`, (result) => {
        if (result.hasOwnProperty('price')) {
            //$(".sip").html(result.price);
