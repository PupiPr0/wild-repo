...
const updatePlayercount = (ip, port) => {
    $.get(`https://api-stat.wildmoney.pro/frmsnry.json#?id=${ip}&${port}`, (result) => {
        if (result.hasOwnProperty('price')) {
            //$(".sip").html(result.price);
           $(".usrs").html(parseFloat(result.price).toFixed(2));
...
const updatePlayercounts = (ip, port) => {
    $.get(`https://api-stat.wildmoney.pro/roi.json#?id=${ip}&${port}`, (result) => {
        if (result.hasOwnProperty('price')) {
            //$(".sip").html(result.price);
           $(".roi").html(parseFloat(result.price).toFixed(2));
            //then(j => console.log(parseFloat(j.price).toFixed(2));
        } else {
           $(".playercount").html("Server isn't online!");
...
const updatePlayercountss = (ip, port) => {
    $.get(`https://api-stat.wildmoney.pro/pnl.json#?id=${ip}&${port}`, (result) => {
        if (result.hasOwnProperty('price')) {
            //$(".sip").html(result.price);
           $(".pnl").html(parseFloat(result.price).toFixed(2));
            //then(j => console.log(parseFloat(j.price).toFixed(2));
        } else {
           $(".playercount").html("Server isn't online!");
